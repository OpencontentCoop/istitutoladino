...
<script id="tpl-dictionary-results" type="text/x-jsrender">
    ...
    {{for searchHits}}
        <div class="Prose u-layout-prose">
          {{if currentUserIsLoggedIn}}<a href="/openpa/object/{{:metadata.id}}" class="pull-right"><i class="fa fa-link"></i></a>{{/if}}
...
<script>
  ...
  $(document).ready(function () {
    $('[data-dictionary_subtree]').each(function () {
      var currentUserIsLoggedIn = {/literal}{cond(fetch(user,current_user).is_logged_in, 'true', 'false')}{literal};
      var baseUrl = '/';
